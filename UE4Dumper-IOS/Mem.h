...
bool vmreadData(void* buf,long address,int length)
{
    vm_size_t size = 0;
    kern_return_t error = vm_read_overwrite(task, (vm_address_t)address, (vm_size_t)length, (vm_address_t)buf, &size);
...
kaddr getRealOffset(kaddr offset) {
    if (libbase == 0) {
        printf("Error: Can't Find Base Addr for Real Offset");
        return 0;
    }
...
T Read(kaddr address) {
    long aaa = 0;
    vmreadData(&aaa, (long)address, sizeof(T));
    return aaa;
}
...
string ReadStr(kaddr address, unsigned int size) {
    ...
    for (int i = 0; i < size; i++) {
        vmreadData(&data[0] + i, (long)address+(sizeof(char) * i), sizeof(char));
