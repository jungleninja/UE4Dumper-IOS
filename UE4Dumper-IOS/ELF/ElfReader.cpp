...
bool ElfReader::ReadElfHeader() {
    ...
    if (rc < 0) {
        printf("can't read file \"%s\": %s", name_, strerror(errno));
        return false;
    }
    if (rc != sizeof(header_)) {
        printf("\"%s\" is too small to be an ELF executable", name_);
        return false;
    }
    return true;
}
...
bool ElfReader::VerifyElfHeader() {
    if (header_.e_ident[EI_MAG0] != ELFMAG0 ||
        ...
        printf("\"%s\" has bad ELF magic", name_);
        return false;
    }
#ifndef __SO64__
    if (header_.e_ident[EI_CLASS] != ELFCLASS32) {
        printf("\"%s\" not 32-bit: %d", name_, header_.e_ident[EI_CLASS]);
        return false;
    }
#else
    if (header_.e_ident[EI_CLASS] != ELFCLASS64) {
        printf("\"%s\" not 64-bit: %d", name_, header_.e_ident[EI_CLASS]);
        return false;
    }
#endif

    if (header_.e_ident[EI_DATA] != ELFDATA2LSB) {
        printf("\"%s\" not little-endian: %d", name_, header_.e_ident[EI_DATA]);
        return false;
    }

    if (header_.e_type != ET_DYN) {
        printf("\"%s\" has unexpected e_type: %d", name_, header_.e_type);
        return false;
    }

    if (header_.e_version != EV_CURRENT) {
        printf("\"%s\" has unexpected e_version: %d", name_, header_.e_version);
        return false;
    }
...
bool ElfReader::ReadProgramHeader() {
    ...
    if (phdr_num_ < 1 || phdr_num_ > 65536/sizeof(Elf_Phdr)) {
        printf("\"%s\" has invalid e_phnum: %zu", name_, phdr_num_);
        return false;
    }
    ...
    if(!LoadFileData(mmap_result, phdr_size_, header_.e_phoff)) {
        printf("\"%s\" has no valid phdr data", name_);
        return false;
    }
...
bool ElfReader::ReserveAddressSpace() {
    ...
    if (load_size_ == 0) {
        printf("\"%s\" has no loadable segments", name_);
        return false;
    }
    ...
    for (size_t i = 0; i < phdr_num_; ++i) {
        ...
        if (file_length != 0) {
            ...
            if(!LoadFileData(load_point, file_length, file_page_start)) {
                printf("couldn't map \"%s\" segment %zu: %s", name_, i, strerror(errno));
                return false;
            }
...
bool ElfReader::FindPhdr() {
    ...
    printf("can't find loaded phdr for \"%s\"", name_);
    return false;
}
...
bool ElfReader::CheckPhdr(uint8_t * loaded) {
    ...
    printf("\"%s\" loaded phdr %p not in loadable segment", name_, loaded);
    return false;
}
...
bool ElfReader::LoadFileData(void *addr, size_t len, int offset) {
    ...
    if (rc != len) {
        printf("\"%s\" has no enough data at %x:%zx, not a valid file or you need to dump more data", name_, offset, len);
        return false;
    }
    return true;
}
